...
Qualifiers allow the effective value of a configuration object to vary based on
the value of a contextFilter.

Each qualifier has a filter and a value. The qualifier filter acts as the
matchFilter for filter comparison purposes.

Where the filters match, the qualifier value is used as an additional object
template to the nominal value of the configuration object. More than one
qualifier may match, in which case the resulting object templates are applied to
the nominal value of the configuration object being qualified in the order in
which the qualifiers are defined.

The way in which the nominal value is modified is controlled by the
"DefaultBehaviour" of the qualifier. Typically this means simple values will be
replaced and for objects, the qualifier value is merged into the default value.

One or more qualifiers can be added to any configuration object via a
framework "Qualifiers" attribute. Where the entity to be qualified is not itself
an object, the desired entity is wrapped in an object in order that qualifiers
can be attached. In this case, the nominal value should be provided via a
"Value" attribute at the same level as the "Qualifiers" attribute.

There is a short form and a long form for qualifiers.

In the short form, the "Qualifiers" entity is an object and each attribute
represents a qualifier. The attribute key is the value of the "Any"
attribute of the matchFilter, and the MatchBehaviour is
FILTER_ANY_MATCH_BEHAVIOUR.

The qualifier value is the value of the attribute. Because
object attribute processing is not ordered, the short form does not provide fine
...
In the long form, the "Qualifiers" entity is a list of qualifier objects. Each
qualifier object must have a "Filter" attribute and a "Value" attribute, as well
as optional "MatchBehaviour" and "DefaultBehaviour" attributes. By default, the
MatchBehaviour is FILTER_ONETOONE_MATCH_BEHAVIOUR.
    ...
    "Qualifiers : [
        {
            ...
            "MatchBehaviour" : FILTER_ONETOONE_MATCH_BEHAVIOUR
        }
    ]
}
--]

[#assign frameworkObjectAttributes += ["Qualifiers"]]

[#function qualifyEntity entity contextFilter]
    [#-- Nothing to do if not qualified --]
    [#if !(entity?is_hash && entity.Qualifiers??)]
        [#return entity]
    [/#if]

    [#local qualifiers = entity.Qualifiers]
    [#local result = removeObjectAttributes(entity, "Qualifiers")]
    [#-- Standardise so nominal value always in "Value" attribute --]
    [#if !(result.Value??)]
    ...
    [#if qualifiers?is_hash]
        [#list qualifiers as anyFilter,anyValue]
            [#if filterMatch(contextFilter, {"Any" : anyFilter}, FILTER_ANY_MATCH_BEHAVIOUR)]
                [#local result = mergeObjects(result, {"Value" : anyValue})]
            [/#if]
        [/#list]
    [/#if]
    ...
    [#if qualifiers?is_sequence]
        [#list qualifiers as qualifier]
            [#local matchBehaviour = qualifier.MatchBehaviour!FILTER_ONETOONE_MATCH_BEHAVIOUR]
            [#if filterMatch(contextFilter, qualifier.Filter!{}, matchBehaviour)]
                [#local result = mergeObjects(result, qualifier.Value!{})]
            [/#if]
        [/#list]
    [/#if]
